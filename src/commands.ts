...
export const commands: Command[] = [
  { name: "/model", usage: "/model [id]", help: "Show or switch the active model", run: cmdModel },
  ...
  { name: "/stats", help: "Show session stats (context, tokens, cost)", run: cmdStats },
  { name: "/tools", help: "List registered tools with risk levels", run: cmdTools },
  { name: "/extensions", help: "List loaded extensions and their tools", run: cmdExtensions },
...
export async function dispatchCommand(cmd: string, args: string): Promise<boolean> {
...
export function printHelp() {
  ...
  console.log(`  ${cyan}${"/abort".padEnd(W)}${reset} Cancel the current agent turn`);
  console.log("");
}
...
export async function cmdMessages() {
  const result = await requestResponse("getMessages") as { messages: Array<Record<string, unknown>> };
  ...
  for (const msg of result.messages) {
    ...
    const roleColor = role === "user" ? cyan : role === "assistant" ? green : role === "system" ? yellow : gray;
...
export async function cmdSessions() {
  const result = await requestResponse("listSessions") as { sessions: Array<{ path: string; mtimeSecs: number; sizeKb: number; entryCount: number }> };
  ...
  for (const s of result.sessions) {
    ...
    console.log(`  ${bold}${s.path}${reset} ${gray}${s.entryCount} entries · ${s.sizeKb}KB · ${date}${reset}`);
  }
}

export async function cmdExtensions() {
  const result = await requestResponse("listExtensions") as { extensions: Array<{ name: string; tools: string[] }> };
  if (result.extensions.length === 0) {
    console.log(`${gray}no extensions loaded${reset}`);
    return;
  }
  for (const ext of result.extensions) {
    console.log(`  ${bold}${ext.name}${reset} ${gray}(${ext.tools.length} tool${ext.tools.length !== 1 ? "s" : ""}${ext.tools.length > 0 ? ": " + ext.tools.join(", ") : ""})${reset}`);
  }
}

export async function cmdTools() {
  const result = await requestResponse("listTools") as { tools: Array<{ name: string; description: string; risk: string }> };
  if (result.tools.length === 0) {
    console.log(`${gray}no tools registered${reset}`);
    return;
  }
  for (const t of result.tools) {
    const riskColor = t.risk === "destructive" ? red : t.risk === "network" ? yellow : t.risk === "write" ? yellow : green;
    console.log(`  ${bold}${t.name}${reset} ${riskColor}[${t.risk}]${reset} ${dim}${t.description}${reset}`);
  }
}
...
export async function cmdStats() {
  ...
  console.log(
    `  ${bold}Context${reset}   ${k(s.estimatedUsed)} / ${k(budget)} (${s.utilizationPercent}%) ` +
    `${gray}[${k(s.completionReserve)} reserved · ${k(s.estimatedRemaining)} left]${reset}`,
  );
  ...
  if (r && (r.system + r.user + r.assistant + r.tool) > 0) {
    console.log(
      `  ${bold}By role${reset}     ${gray}system ${k(r.system)} · user ${k(r.user)} · assistant ${k(r.assistant)} · tool ${k(r.tool)}${reset}`,
    );
  }
  ...
  console.log(`  ${bold}Messages${reset}  ${s.messageCount} messages · ${s.entryCount} entries${compacted}`);
  ...
  const cost = a && a.totalCost > 0
    ...
    : a && a.requestCount > 0 ? `${gray}cost n/a${reset}` : `${gray}—${reset}`;
  const cached = a && a.totalCachedTokens > 0 ? ` · ${k(a.totalCachedTokens)} cached` : "";
  console.log(
    `  ${bold}API${reset}       ${a.requestCount} requests · ${k(a.totalTokens)} tokens${cached} · ${cost}`,
  );
}
...
export async function cmdResume(args: string) {
  ...
  try {
    const result = await requestResponse("resumeSession", { path: args }) as { path: string; model: string; cwd: string; entryCount: number };
    setCurrentModel(result.model);
    console.log(`${green}resumed${reset} ${result.path} ${gray}(${result.entryCount} entries, model: ${result.model})${reset}`);
  } catch (e) {
    const message = (e as { message?: string })?.message ?? JSON.stringify(e);
